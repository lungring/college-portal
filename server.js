...
// Initialize the Google Gen AI client with your active key securely from Render
...
// NEW: Create Reviews & Comments Table
db.exec(`
...
// Insert Mock Colleges
const insertCollege = db.prepare('INSERT INTO colleges (name, rank, fees, exam, cutoff, streams, placement, infrastructure, url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
...
// Insert some Initial Mock Reviews
const insertReview = db.prepare('INSERT INTO reviews (college_id, user_name, rating, comment) VALUES (?, ?, ?, ?)');
insertReview.run(3, 'Rohan_Das', 5, 'JU has the best return on investment in the country. The fees are exceptionally low!');
insertReview.run(1, 'Ananya_Sharma', 4, 'IIT Bombay placements are legendary, but the academic pressure is massive.');
...
// NEW: Get Reviews for a specific college
app.get('/api/reviews/:collegeId', (req, res) => {
  try {
    const getReviewsQuery = db.prepare(`SELECT * FROM reviews WHERE college_id = ? ORDER BY timestamp DESC`);
    const reviews = getReviewsQuery.all(req.params.collegeId);
    res.json({ reviews });
  } catch (error) {
...
// NEW: Post a Review/Comment for a college
app.post('/api/reviews', (req, res) => {
  const { college_id, user_name, rating, comment } = req.body;
  if (!user_name || !rating || !comment) {
    return res.status(400).json({ error: 'All fields are required' });
  }
  try {
    const statement = db.prepare(`INSERT INTO reviews (college_id, user_name, rating, comment) VALUES (?, ?, ?, ?)`);
    statement.run(college_id, user_name, rating, comment);
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        .college-card { border: 1px solid #dadce0; border-radius: 8px; padding: 25px; background: white; transition: box-shadow 0.2s; border-left: 6px solid #0070f3; margin-bottom: 20px; }
        ...
        /* Review Styling UI */
        .review-section { border-top: 1px dashed #cbd5e1; padding-top: 15px; margin-top: 15px; background: #fafafa; padding: 15px; border-radius: 6px; }
        .review-title { font-size: 14px; font-weight: bold; color: #334155; margin-bottom: 10px; }
        .review-item { font-size: 13px; border-bottom: 1px solid #f1f5f9; padding: 8px 0; }
        .review-item:last-child { border: none; }
        .review-form { display: flex; gap: 8px; margin-top: 12px; flex-wrap: wrap; }
        .review-form input, .review-form select { padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px; font-size: 12px; outline: none; }
        .review-form input.comment-input { flex: 1; min-width: 200px; }
        .btn-sm { background: #334155; color: white; border: none; padding: 8px 14px; border-radius: 4px; font-size: 12px; cursor: pointer; }
          ...
          <div class="dashboard-blueprint">
            <div class="blueprint-card" style="border-left: 5px solid #2563eb;">
              ...
              <ol>
                ...
                <li><strong>Leave Feedback:</strong> Scroll to the bottom of any college card to write reviews and leave ratings.</li>
              </ol>
            </div>
            <div class="blueprint-card" style="border-left: 5px solid #b99110;">
              <h3>⚡ Engine Features & Frameworks</h3>
              <ul>
                <li><strong>Gemini AI Integration Module:</strong> Intercepts context parameters to answer forum boards.</li>
                <li><strong>Dynamic Review Pipeline:</strong> Updates active reviews dynamically via persistent endpoint payloads.</li>
              </ul>
            </div>
          </div>
      ...
      <script>
        ...
        async function performSearchFetch() {
          ...
          for (const college of data.colleges) {
            ...
            // Fetch reviews dynamically for this college
            const reviewRes = await fetch('/api/reviews/' + college.id);
            ...
            reviewData.reviews.forEach(r => {
              reviewsHTML += \`
                <div class="review-item">
                  <strong>👤 \${r.user_name}</strong> (\${'⭐'.repeat(r.rating)}) - <span style="color:#475569;">\${r.comment}</span>
                </div>
              \`;
            });
            if (reviewData.reviews.length === 0) {
              reviewsHTML = '<p style="font-size:12px; color:#94a3b8; margin:0;">No student comments left yet. Be the first!</p>';
            }
            ...
            feed.innerHTML += \`
              <div class="college-card">
                ...
                <div class="review-section">
                  <div class="review-title">📣 Student Comments & Reviews (\${reviewData.reviews.length})</div>
                  <div id="reviews-box-\${college.id}">\${reviewsHTML}</div>
                  
                  <div class="review-form">
                    <input type="text" id="rev-name-\${college.id}" placeholder="Your Handle (e.g. Alum_CSE)" style="width:130px;">
                    <select id="rev-rate-\${college.id}">
                    ...
                    <input type="text" id="rev-comm-\${college.id}" class="comment-input" placeholder="Leave a public campus review insight...">
                    <button class="btn-sm" onclick="submitCollegeReview(\${college.id})">Post Review</button>
                  </div>
                </div>
              </div>
            \`;
        ...
        async function submitCollegeReview(collegeId) {
          const nameInput = document.getElementById('rev-name-' + collegeId);
          const rateSelect = document.getElementById('rev-rate-' + collegeId);
          const commInput = document.getElementById('rev-comm-' + collegeId);
          ...
          if (!userName || !comment) {
            alert('Please fill out your name and a comment review payload before submitting!');
            return;
          }
          ...
          const res = await fetch('/api/reviews', {
            ...
            body: JSON.stringify({ college_id: collegeId, user_name: userName, rating: rating, comment: comment })
          });
          
          if (res.ok) {
            commInput.value = '';
            nameInput.value = '';
            performSearchFetch(); // Reload display layout to render new comment instantly
          }
        }
...
// Dynamic binding for both your MacBook (Localhost) and Render environments
